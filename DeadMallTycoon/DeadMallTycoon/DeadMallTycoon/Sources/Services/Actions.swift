...
enum ArtifactActions {
    ...
    // memoryWeight / thoughtReferenceCount / yearCreated / condition). Only
    // the type, thoughtTriggers, and displayContent change. This preserves
    // the memorial's accumulated provenance — a sealed or displayed slot
    // remembers everything the boarded slot did.
    //
    // Gating:
    ...
    //
    // Invalid calls are clean no-ops (return state unchanged). UI never
    // offers an ineligible verb in the inspector, so these paths are
    // defensive. Each successful action appends a LedgerEntry.
